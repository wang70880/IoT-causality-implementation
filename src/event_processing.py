	...
	def partition_data_frame(self, transition_events_states):
		...
		for seg_point in seg_points: # Get the data frame with range [last_point, seg_point]
			...
			if testing_start_point-last_point < len(self.attr_names) or seg_point - testing_start_point < len(self.attr_names):
				print("(last_point, seg_point={}, {}) The current dataframe contains too few records! Training: {}. Testing: {}."\
						.format(last_point, seg_point, testing_start_point-last_point, seg_point - testing_start_point))
				exit()
